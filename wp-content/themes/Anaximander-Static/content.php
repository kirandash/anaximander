...
<?php $post_count = $wp_query->post_count; // Get the total post count ?>

	
<?php 
	if ($post_count >= 2) {
		if( ($post == $posts[1]) && ( is_home() && !is_paged()) && is_active_sidebar( 'front-widget-1' ) ) { 

			echo '<article class="group index-post-container index-widget-area">';
			dynamic_sidebar( 'front-widget-1' );
			echo '</article>';
		} 
	}

	if ($post_count >= 7) {
		if( ($post == $posts[6]) && ( is_home() && !is_paged()) && is_active_sidebar( 'front-widget-2' ) ) { 
			
			echo '<article class="group index-post-container index-widget-area">';
			dynamic_sidebar( 'front-widget-2' );
			echo '</article>';
		}
	}
	
	if ($post_count >= 10) {
		if( ($post == $posts[9]) && ( is_home() && !is_paged()) && is_active_sidebar( 'front-widget-3' ) ) { 
			echo '<article class="group index-post-container index-widget-area">';
			dynamic_sidebar( 'front-widget-3' );
			echo '</article>';
		} 
	}
?>
